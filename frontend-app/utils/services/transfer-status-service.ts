...
export async function fetchTransferStatusFromBackend(params: {
  ...
  if (withdrawalRes.ok) {
    ...
    return {
      ...
      transfer_completed: Boolean(body.transfer_completed),
      amount: body.amount,
  ...
  // 2) Fallback: get all transfers for account and find our transfer ID
  const transfersRes = await fetch(
    `${backendUrl}/api/account/${safeAccountId}/transfers?limit=50&direction=INCOMING`,
    {
      method: 'GET',
      headers: {
        'X-API-Key': backendApiKey,
