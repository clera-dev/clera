...
export function generateScenarioPrompt(scenario: string, context: PromptContext): string {
  ...
  return scenarios[scenario as keyof typeof scenarios] || generateContextualPrompt(context);
}
...
export function generateTimelinePrompt(timeline: string, context: PromptContext): string {
  ...
  return timelinePrompts[timeline as keyof typeof timelinePrompts] || generateContextualPrompt(context);
}
...
export function generateActionBasedPrompt(action: string, symbol?: string): string {
  ...
  return actionPrompts[action as keyof typeof actionPrompts] || 'Can you help me understand how to research and evaluate investment opportunities?';
}
