...
export async function POST(
  ...
  try {
    ...
    // Call backend API to cancel orders
    console.log(`Cancelling orders for account closure: ${accountId}`);
    const response = await fetch(`${backendUrl}/account-closure/cancel-orders/${accountId}`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error(`Order cancellation error: ${errorDetail}`);
      return NextResponse.json(
        { detail: errorDetail },
