...
export async function POST(
  ...
  try {
    ...
    // Call backend API to check settlement
    console.log(`Checking settlement status for account closure: ${accountId}`);
    const response = await fetch(`${backendUrl}/account-closure/settlement-status/${accountId}`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error(`Settlement check error: ${errorDetail}`);
      return NextResponse.json(
        { detail: errorDetail },
