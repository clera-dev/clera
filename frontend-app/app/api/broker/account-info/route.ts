...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`Account Info API: Getting account info for account: ${accountId}, user: ${user.id}`);
    ...
    if (onboardingError || !onboardingData) {
      console.error(`Account Info API: User ${user.id} does not own account ${accountId}`);
      return NextResponse.json(
    ...
    console.log(`Account Info API: Ownership verified. User ${user.id} owns account ${accountId}`);
