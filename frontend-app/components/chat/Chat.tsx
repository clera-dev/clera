...
import { Message as LangGraphMessage, Interrupt } from '@langchain/langgraph-sdk';
import { 
  saveChatHistory, 
  loadChatHistory,
  formatChatTitle,
...
type InterruptValueType = string;
...
// Enhanced ChatSkeleton component with tool status
interface ChatSkeletonWithStatusProps {
  toolStatus?: string;
}

function ChatSkeletonWithStatus({ toolStatus }: ChatSkeletonWithStatusProps) {
  ...
  return (
    <div className="flex w-full items-start gap-4">
      ...
      <div className="rounded-lg px-4 py-3 bg-muted min-h-[40px] min-w-[120px] flex items-center">
        <div className="text-sm text-muted-foreground">
          {toolStatus ? (
            <span className="text-primary">{toolStatus}</span>
          ) : (
...
export default function Chat({ 
  ...
  // Standard useStream hook for messages with custom event handling
  const thread = useStream<GraphStateType, { InterruptType: InterruptValueType }>({
    ...
    onCustomEvent: (event: any) => {
      ...
      if (event && typeof event === 'object') {
        if ((event as any).type === 'tool_update') {
  ...
  // Clear tool status when processing completes
  useEffect(() => {
    if (!thread.isLoading && !isCreatingSession) {
      setCurrentToolStatus('');
    }
  }, [thread.isLoading, isCreatingSession]);
  ...
  useEffect(() => {
    if (currentThreadId && pendingFirstMessage && !isProcessing) {
  ...
  }, [currentThreadId, pendingFirstMessage, thread, userId, accountId, onMessageSent, onQuerySent, isProcessing]);
  ...
  }, [input, isProcessing, isInterrupting, thread, userId, accountId, currentThreadId, onSessionCreated, onMessageSent, onQuerySent]);
  ...
  // Update internal state when the session ID prop changes
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      >
        ...
        {isProcessing && !isInterrupting && <ChatSkeletonWithStatus toolStatus={currentToolStatus} />}
