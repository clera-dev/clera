...
def get_account_id(state=None, config=None) -> str:
    ...
    logger.info(f"[Trade Agent] get_account_id called. Received config: {config}, state: {state}")

    # ---- STRATEGY 1: Use Config ----
    if config and isinstance(config, dict) and isinstance(config.get('configurable'), dict):
        logger.info("[Trade Agent] Config is a valid dictionary with 'configurable' key.")
        ...
        if current_account_id:
            ...
            if current_user_id: 
                ...
                logger.info(f"[Trade Agent] Stored user_id from config: {current_user_id}")
            return current_account_id
        if current_user_id:
            _LAST_VALID_USER_ID = current_user_id 
            logger.info(f"[Trade Agent] User ID found in config: {current_user_id}, will try Supabase lookup.")
        else:
            logger.info("[Trade Agent] Config had 'configurable', but no account_id or user_id found.")
    else:
        logger.info("[Trade Agent] Config is NOT a valid dictionary or lacks 'configurable'. Skipping Strategy 1.")

    # ---- STRATEGY 2: Use State (if available and needed) ----
    if state and isinstance(state, dict):
        logger.info("[Trade Agent] State is a valid dictionary. Checking for account_id/user_id.")
        ...
        if state_account_id and not current_account_id:
            ...
            if state_user_id: 
                ...
                logger.info(f"[Trade Agent] Stored user_id from state: {state_user_id}")
        ...
        if state_user_id and not current_user_id: 
            ...
            logger.info("[Trade Agent] State did not provide needed IDs (or they were already found in config).")
    else:
        logger.info("[Trade Agent] State is not a valid dictionary or not provided. Skipping Strategy 2.")
